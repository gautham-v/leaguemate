...
export function UpgradeModal({ leagueCount, onClose }: UpgradeModalProps) {
  ...
  const [view, setView] = useState<'upgrade' | 'signin' | 'checkout'>('upgrade');
  ...
  const handleGoogleSignIn = async () => {
    ...
    await createClient().auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${process.env.NEXT_PUBLIC_APP_URL || 'https://leaguemate.fyi'}/auth/callback?next=${next}`,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div
        className={`w-full bg-card-bg border border-card-border rounded-2xl shadow-2xl transition-all duration-300 flex flex-col max-h-[90vh] ${
          isCheckout ? 'max-w-lg' : 'max-w-sm'
        }`}
      >
        {/* Sticky header — always visible */}
        <div className="relative px-5 pt-5 pb-4 border-b border-card-border/60 shrink-0">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <h2 className="text-base font-bold text-white">
              {isCheckout ? 'Complete your purchase' : 'Unlock all your leagues'}
            </h2>
          </div>
          {!isCheckout && (
